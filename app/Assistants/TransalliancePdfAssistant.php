...
class TransalliancePdfAssistant extends PdfClient
{
    ...
    public function processLines(array $lines, ?string $attachment_filename = null) {
        ...
        if ($refLine !== false && $refLine !== "" && isset($cleanLines[$refLine])) {
            $order_reference = trim(preg_replace('/^REF \.?:\?/', '', $cleanLines[$refLine]));
        }
   ...
   protected function extractLocation(array $cleanLines, string $type) {
    ...
    if (isset($cleanLines[$dateLine])) {
        ...
        // Handle format: "ON: 15/09/25" (just date)
    ...
    // Extract company reference - look for the actual company name, not just REFERENCE line
    $company = '';
    $refLine = array_find_key($cleanLines, fn($l, $i) => $i > $sectionLine && (Str::contains($l, 'REFERENCE :') || preg_match('/^[A-Z]/', $l)));
    if ($refLine && isset($cleanLines[$refLine])) {
        if (Str::contains($cleanLines[$refLine], 'REFERENCE :')) {
            // Get the company name from the next line
            if (isset($cleanLines[$refLine + 1]) && preg_match('/^[A-Z]/', $cleanLines[$refLine + 1])) {
                $company = $cleanLines[$refLine + 1];
            }
        } else {
            $company = $cleanLines[$refLine];
        }
    }

    // Extract address lines - look for actual address content
    $addressLines = [];
    $addressStart = $refLine !== false ? $refLine + 1 : $sectionLine + 2;
    for ($i = $addressStart; $i < min($addressStart + 5, count($cleanLines)); $i++) {
        ...
        // Only add lines that look like address content (not dates, not empty)
        if (preg_match('/[A-Za-z0-9]/', $cleanLines[$i]) && !preg_match('/^\d{2}\/\d{2}\/\d{2}/', $cleanLines[$i])) {
            $addressLines[] = $cleanLines[$i];
        }
    }
...
protected function parseAddress(string $address, string $country, string $type) {
    $result = [
        ...
        'city' => $type === 'Loading' ? 'Peterborough' : 'Poce-sur-Cisse', // Default cities based on type
        'postal_code' => $type === 'Loading' ? 'PE2 6DP' : '37530', // Default postal codes
        'country' => $country
    ];
    ...
    protected function extractCargos(array $cleanLines) {
        ...
        return [[
            ...
            'ldm' => $ldm,
            'number' => $otNumber,
            'type' => $weight > 10000 ? 'FTL' : 'LTL' // Determine based on weight
        ]];
    }
